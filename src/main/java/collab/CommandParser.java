...
public class CommandParser {
    ...
    protected static final List<String> WHITE_COMMAND_LIST = Arrays.asList(ADD_COMMAND,
        SEARCH_COMMAND, MODIFY_COMMAND, DELETE_COMMAND);
    ...
    public ICommand parseToCommand(String cmdStr) throws Exception {
        ...
        return getCommandFromToken(getFirstOptionFromToken(tokenList),
            getSecondOptionFromToken(tokenList), getThirdOptionFromToken(tokenList), tokenList);
    }

    private void checkValidation(String[] tokenList) throws Exception {
        if (tokenList.length < 1) {
            throw new Exception(ERR_MSG_FORMAT_WRONG);
        }
        if (!WHITE_COMMAND_LIST.contains(tokenList[0])) {
            throw new Exception(ERR_MSG_CMD_INVALID);
        }
        if (!FIRST_OPTION_WHITE_LIST.contains(tokenList[1])) {
            throw new Exception(ERR_MSG_OPTION_WRONG);
        }
        if (!tokenList[2].equals(" ")) {
            checkSecondOptionValidation(tokenList);
        }
        if (tokenList[0].equals(ADD_COMMAND)) {
            checkAddCommandValidation(tokenList);
        }
        if (tokenList[0].equals(SEARCH_COMMAND)) {
            checkSearchCommandValidation(tokenList);
        }
        if (tokenList[0].equals(MODIFY_COMMAND)) {
            checkModifyCommandValidation(tokenList);
        }
        if (tokenList[0].equals(DELETE_COMMAND)) {
            checkDeleteCommandValidation(tokenList);
        }
    }
    ...
    private void checkSecondOptionValidation(String[] tokenList) throws Exception {
        ...
        if (!SECOND_OPTION_WHITE_MAP.containsKey(secondOption)) {
            throw new Exception(ERR_MSG_OPTION_WRONG);
        }
        if (!SECOND_OPTION_WHITE_MAP.get(secondOption).contains(FirstArgument)) {
            throw new Exception(ERR_MSG_OPTION_WRONG);
        }
    }

    private void checkAddCommandValidation(String[] tokenList) throws Exception {
        if (tokenList.length != ADD_CMD_LENGTH) {
            throw new Exception(ERR_MSG_FORMAT_WRONG);
        }
    ...
    private AbstractSecondOption getSecondOptionFromToken(String[] tokenList) {
        ...
        if (secondOption.equals("-f")) {
            return new FirstNameOption(optionArguments);
        }
        if (secondOption.equals("-l")) {
            return (firstArgument.equals("name") ? new LastNameOption(optionArguments)
                : new LastPhoneNumberOption(optionArguments));
        }
        if (secondOption.equals("-m")) {
            return (firstArgument.equals("phoneNum") ? new MiddlePhoneNumberOption(optionArguments)
                : new MonthOfBirthdayOption(optionArguments));
        }
        if (secondOption.equals("-y")) {
            return new YearOfBirthdayOption(optionArguments);
        }
        if (secondOption.equals("-d")) {
            return new DayOfBirthdayOption(optionArguments);
        }
    ...
    ICommand getCommandFromToken(AbstractFirstOption FirstOption, AbstractSecondOption SecondOption,
        AbstractThirdOption ThirdOption, String[] tokenList) throws Exception {
        String command = tokenList[0];
        if (command.equals(ADD_COMMAND)) {
            return new AddCommand(Arrays.asList(Arrays.copyOfRange(tokenList, 4, ADD_CMD_LENGTH)));
        }
        if (command.equals(SEARCH_COMMAND)) {
            return new SearchCommand(FirstOption, SecondOption);
        }
        if (command.equals(MODIFY_COMMAND)) {
            return new ModifyCommand(FirstOption, SecondOption,
                Arrays.asList(Arrays.copyOfRange(tokenList, 6, MOD_CMD_LENGTH)));
        }
        if (command.equals(DELETE_COMMAND)) {
            return new DeleteCommand(FirstOption, SecondOption);
        }
        throw new Exception(ERR_MSG_CMD_INVALID);
    }
}
