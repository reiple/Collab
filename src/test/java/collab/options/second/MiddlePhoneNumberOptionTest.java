...
class MiddlePhoneNumberOptionTest {
  ...
  @Test
  void 전화번호_중간자리_검색실패_테스트() {

    MiddlePhoneNumberOption option2 = new MiddlePhoneNumberOption(Arrays.asList("phoneNum", "9777"));
  ...
  @Test
  void 전화번호_중간자리_검색성공_테스트() {

    MiddlePhoneNumberOption option2 = new MiddlePhoneNumberOption(Arrays.asList("phoneNum", "7174"));

    List<Employee> filteredList = option2.getFilteredList(dao);
    assertEquals(1, filteredList.size());

    // {"01122329", "DN WD", "CL4", "010-7174-5680", "20071117", "PRO"},
    assertEquals("010-7174-5680", filteredList.get(0).getPhoneNumber());
    assertEquals("01122329", filteredList.get(0).getEmployeeNumber());
    assertEquals("DN WD", filteredList.get(0).getName());
    assertEquals("CL4", filteredList.get(0).getCareerLevel());
    assertEquals("20071117", filteredList.get(0).getBirthday());
    assertEquals("PRO", filteredList.get(0).getCerti());

  }

  @Test
  void 전화번호_중간자리_검색성공_여러개_테스트() {

    MiddlePhoneNumberOption option2 = new MiddlePhoneNumberOption(Arrays.asList("phoneNum", "8900"));
