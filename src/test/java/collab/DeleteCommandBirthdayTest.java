...
class DeleteCommandBirthdayTest {
  ...
  void setupMock() {
    ...
    when(employeeDAO.deleteItemById(anyString())).thenReturn(new Employee(Arrays.asList("00114058", "QQWE LVARW", "CL4", "010-4328-3059", "19911021", "PRO")));
  }
  ...
  @Test
  void testSearchBirthdayAndDeleteAndPrintFail() throws Exception {
    ICommand command = new DeleteCommand(
        new PrintOption(), new EmptySecondOption(Arrays.asList("birthday", "20071101"))
    );
  ...
  @Test
  void testSearchBirthdayAndDeleteAndPrintSuccess() throws Exception {
    ICommand command = new DeleteCommand(
        new PrintOption(), new EmptySecondOption(Arrays.asList("birthday", "19911021"))
    );
    ...
    String result = command.executeCommand(employeeDAO);
    assertEquals(ResultStringMaker.makeResultString("DEL", data), result);

    ICommand sameCommand = new DeleteCommand(
        new PrintOption(), new EmptySecondOption(Arrays.asList("birthday", "19911021"))
    );
